function hx --description 'Helix: IDE-режим в zellij (Discord presence сохраняется)'
    ...
    if status is-interactive; and test -z "$ZELLIJ"
        ...
        # Воркер перекраски фона окна kitty (поля/зазор) в такт теме Helix.
        # Живёт ЗДЕСЬ, в shell kitty (вне zellij), потому что OSC 11 доходит до
        # самого окна только со своей tty — из панели zellij его не пропускает.
        set -l gapwatch
        set -l gappy "$HOME/.local/share/hx-presence/venv/bin/python"
        set -l gapscript "$HOME/.local/share/hx-presence/kitty_gap.py"
        if test -x "$gappy"; and test -f "$gapscript"
            "$gappy" "$gapscript" &
            set gapwatch $last_pid
        end
        env HX_ARGS="$argv" zellij --layout hx-ide
        test -n "$gapwatch"; and kill $gapwatch 2>/dev/null
